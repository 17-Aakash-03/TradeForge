...
def register(request: Request, body: RegisterRequest,
    ...
    result = register_user(body.email, body.username,
                           body.password, db)
    if "error" in result:
...
def upgrade_to_pro(body: UpgradeRequest,
                   db: Session = Depends(get_db)):
    ...
    db.commit()
    return {
        "message": f"{user.email} upgraded to Pro tier",
        "tier": user.tier
    }
...
def predict(request: Request, ticker: str,
            ...
            db: Session = Depends(get_db)):
    from app.services.feature_engineer import get_features_for_ticker
    from app.services.predictor import (predict_lstm,
                                        predict_xgboost,
                                        predict_ensemble)
    if model == "xgboost" and current_user.tier != "pro":
        raise HTTPException(
            status_code=403,
            detail="XGBoost model requires Pro tier"
        )
    df = get_features_for_ticker(ticker, db)
    if df.empty:
        return {"error": f"No data found for {ticker}"}
...
def backtest(ticker: str,
    ...
    if df.empty:
        return {"error": f"No data found for {ticker}"}
    result = run_backtest(ticker, df, initial_capital=capital)
    return result
